/* ============================================================
   ...
   Shared by server.js (local dev) and api/[...path].js (Vercel).
...
async function fetchFlights() {
  try {
    ...
    // server-to-server has no CORS constraints — adsb.lol works fine as a fallback here
    const r = await fetch('https://api.adsb.lol/v2/point/40.71/-74.01/40');
    if (!r.ok) throw new Error('adsb.lol http ' + r.status + ' (after ' + (e.message || e) + ')');
    const j = await r.json();
    ...
    console.log('[flights] opensky failed (' + (e.message || e) + ') → served adsb.lol fallback');
