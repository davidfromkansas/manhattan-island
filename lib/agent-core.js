...
async function feedQuery(input) {
  ...
  const d = await readFeed(def.route);
  const { matched, rows } = applyQuery(def.rows(d), input, def.search);
  return { ...def.summary(d), matched, rows };
}
...
function substrate() {
  ...
  subLoading = (async () => {
    ...
    const [bj, sj] = await Promise.all([read('buildings.json'), read('streets.json')]);
...
async function spatialReport(input) {
  ...
  const rows = def.rows(await readFeed(def.route)).filter(r => typeof r.lat === 'number');
  const report = [];
  for (const a of anchors) {
...
async function buildLayer(input) {
  ...
  const layer = { title, color, points: [], segs: [], rings: [], fit: input.fit !== false };
  ...
  } else return { error: 'source needs feed | buildings | street | points' };
  ...
  if (!layer.points.length && !layer.segs.length && !layer.rings.length) return { error: 'layer came out empty — adjust the query' };
...
const TOOLS = [
  { name: 'get_data',
    description: 'Read a live data feed. filter = case-insensitive substring on names/routes/callsigns (feed=subway: exact route letter like "A"). near {lat,lon,radius_m} keeps rows within radius, adds dist_m, sorts nearest-first. sort_by = any numeric row field (speedMph, bikes, altM, dist_m…), biggest-first unless descending:false. top caps rows.',
    input_schema: { type: 'object', properties: {
      ...
      sort_by: { type: 'string' }, descending: { type: 'boolean' }, top: { type: 'integer' } }, required: ['feed'] } },
  { name: 'get_history',
  ...
  { name: 'show_layer',
    description: 'Draw a result layer on the 3D map (pins + labels, street polylines, buffer rings). The server materializes source queries — do NOT copy rows into points by hand when a source query can produce them. Max 3 layers per turn; a new turn\'s layers replace the old ones. fit:false keeps the camera still.',
    input_schema: { type: 'object', properties: {
      title: { type: 'string' }, color: { type: 'string', enum: PALETTE },
      source: { type: 'object', properties: {
        feed: { type: 'string' }, filter: { type: 'string' },
        near: { type: 'object', properties: { lat: { type: 'number' }, lon: { type: 'number' }, radius_m: { type: 'number' } } },
  ...
  { name: 'set_camera',
    description: 'Fly the user\'s 3D camera. Use preset when one fits; else a lat/lon target (your NYC knowledge is fine) or scene {sx,sz} (from geocode_intersection/query_buildings). altitude_m: ~250 street, ~500 landmark, ~1200 neighborhood, 4000+ borough. heading_deg: compass bearing the camera looks TOWARD (avenues run ~29°E of N; 20–30 reads up the island). Skip it when show_layer fit already frames the result. At most one call per user request.',
    input_schema: { type: 'object', properties: {
...
migration — plus the real street graph (86k segments), 305k real building footprints, and daily \
...
- When the user asks to see/zoom/fly somewhere, call set_camera (preset > coordinates). If a layer \
with fit:true already frames the result, skip set_camera.
- "Around here / what am I looking at" → use the [viewer context] line appended to the message.
...
function contextLine(ctx) { // viewer context from the client, sanitized
  ...
  if (street) bits.push('near ' + street + (boro ? ', ' + boro : ''));
  ...
  if (ctx.focus_kind) { // the dialogue/entity the user last clicked in the scene
    ...
    bits.push(`user is following: ${kind} — ${label}` + (age !== null ? ` (clicked ${age}s ago)` : '') + (data && data !== 'null' ? ' ' + data : ''));
  }
