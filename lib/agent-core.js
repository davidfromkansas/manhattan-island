/* ============================================================
   ...
   Named places/businesses bridge via find_place (Google Places
   Text Search → lat/lon; NYC-restricted, cached, budget-capped).
...
/* ---------- find_place: Google Places (New) Text Search ----------
   Named place/business → lat/lon candidates. Server-side only, behind the
   agent's rate limits — never a public geocode proxy, key never leaves here.
   Cost rails (soft, in-process): Pro-SKU field mask only, 24 h query cache
   (Google TOS allows coords ≤30 days), PLACES_PER_DAY per warm instance,
   429 circuit breaker until next UTC midnight. The HARD $10/month ceiling
   is Google-side: daily quota cap (~170/day) + billing budget on the
   project — in-process counters reset with instances and are not the
   guarantee. Auth: GOOGLE_MAPS_API_KEY env · git-ignored google-maps-key.json. */
const PLACES_URL = 'https://places.googleapis.com/v1/places:searchText';
const PLACES_FIELDS = 'places.displayName,places.formattedAddress,places.location,places.types,places.businessStatus'; // exactly Pro SKU — rating/phone/hours would jump to Enterprise
const PLACES_PER_DAY = 150;           // sits under the ~170/day quota cap in the console
const PLACES_CACHE_TTL = 24 * 3600e3;
const placesCache = new Map();        // normalized query → { t, result }
let placesDay = '', placesCount = 0, placesExhaustedUntil = 0;
...
function placesKey() {
  ...
  try {
    ...
    if (j.key) return j.key;
  } catch { /* no key: tool degrades gracefully */ }
  return null;
}

async function findPlace(input) {
  const q = String((input && input.query) || '').trim().replace(/\s+/g, ' ').slice(0, 120);
  if (!q) return { error: 'query required' };
  const norm = q.toLowerCase();
  const hit = placesCache.get(norm);
  if (hit && Date.now() - hit.t < PLACES_CACHE_TTL) return hit.result;
  const key = placesKey();
  if (!key) return { error: 'place search not configured — fall back to geocode_intersection with the nearest cross-streets you know' };
  const overBudget = 'place search is over its daily budget — fall back to geocode_intersection with the nearest cross-streets you know';
  if (Date.now() < placesExhaustedUntil) return { error: overBudget };
  const today = new Date().toISOString().slice(0, 10);
  if (placesDay !== today) { placesDay = today; placesCount = 0; }
  if (++placesCount > PLACES_PER_DAY) return { error: overBudget };

  let j = null;
  for (let attempt = 0; ; attempt++) { // one retry on timeout/network/5xx, like callGateway
    try {
      const r = await fetch(PLACES_URL, {
        ...
        body: JSON.stringify({
          textQuery: q, pageSize: 5, regionCode: 'US',
          locationRestriction: { rectangle: {
            low: { latitude: NYC.latMin, longitude: NYC.lonMin },
            high: { latitude: NYC.latMax, longitude: NYC.lonMax } } } })
      });
      if (r.status === 429) { // quota exhausted at Google (the hard ceiling working) — stop asking until tomorrow
        placesExhaustedUntil = Date.parse(today + 'T00:00:00Z') + 86_400_000;
        return { error: overBudget };
      }
      if (!r.ok) {
        if (r.status >= 500 && attempt === 0) continue;
        return { error: 'place search failed (' + r.status + ') — fall back to geocode_intersection' };
      }
      j = await r.json();
      break;
    } catch {
      if (attempt === 0) continue;
      return { error: 'place search unavailable right now — fall back to geocode_intersection' };
    }
  }

  const results = [];
  for (const p of (j.places || []).slice(0, 5)) {
    const lat = Number(p.location && p.location.latitude), lon = Number(p.location && p.location.longitude);
    if (!isFinite(lat) || !isFinite(lon)) continue;
    if (lat < NYC.latMin || lat > NYC.latMax || lon < NYC.lonMin || lon > NYC.lonMax) continue; // belt-and-braces under locationRestriction
    const row = { name: ((p.displayName && p.displayName.text) || q).slice(0, 80),
      address: p.formattedAddress ? String(p.formattedAddress).slice(0, 120) : null,
      lat: +lat.toFixed(6), lon: +lon.toFixed(6), types: (p.types || []).slice(0, 4) };
    if (p.businessStatus && p.businessStatus !== 'OPERATIONAL') row.status = String(p.businessStatus).toLowerCase().replace(/_/g, ' ');
    try { // borough + NTA neighborhood — lets the model confirm "les" really matched the Lower East Side
      const w = await whereIs({ lat, lon });
      if (!w.error) { row.borough = w.borough; row.neighborhood = w.neighborhood; }
    } catch { /* annotation is best-effort */ }
    results.push(row);
  }
  if (!results.length) return { error: 'no places matched "' + q + '" in NYC — try adding the neighborhood or cross-streets' };
  const result = { query: q, results, source: 'Google Places' };
  placesCache.set(norm, { t: Date.now(), result });
  if (placesCache.size > 500) placesCache.delete(placesCache.keys().next().value); // evict oldest insert
  return result;
}

/* ---------- tools ---------- */
const NYC = { latMin: 40.45, latMax: 41.05, lonMin: -74.35, lonMax: -73.55 };
const SCENE = { xMin: -15000, xMax: 24000, zMin: -29000, zMax: 27500 }; // full five-borough build: Tottenville z≈-27.3k, north Bronx z≈27k
...
const TOOLS = [
  ...
  { name: 'find_place',
    description: 'Google Places search: a NAMED place/business/landmark/address → real name, address, lat/lon, borough + neighborhood (up to 5 candidates, NYC only). Use for shops, cafes, restaurants, bars, venues, addresses — anything you cannot place by intersection. Expand user shorthand into the query ("Black cat les" → "Black Cat coffee Lower East Side"). Then set_camera to the lat/lon (altitude_m ~250-350, tight highlight_radius_m ~120-200) and drop a labeled show_layer points pin. If candidates are ambiguous, say which one you picked or ask. Data © Google.',
    input_schema: { type: 'object', properties: {
      query: { type: 'string', description: 'place name + any area hint, e.g. "Katz\'s Delicatessen" or "Black Cat coffee Lower East Side"' } },
      required: ['query'] } }
];
...
The scene covers ALL FIVE BOROUGHS, fully built from the city's real 3D building model — Manhattan, \
Brooklyn, Queens, the Bronx, and Staten Island, including Roosevelt/Governors/Rikers/City Islands, \
LGA + JFK, the Staten Island Railway, and lore-accurate shorelines, parks (2k NYC Parks properties), \
lakes (Prospect Park Lake, Meadow/Willow, Baisley, Kissena, Silver, Clove, Wolfe's) and waterways \
(Newtown Creek, Gowanus Canal, Bronx River, Flushing Creek/Bay) — with LIVE feeds: aircraft, subway, \
...
(105k segments citywide), 305k real building footprints, and daily snapshots of the past week.
...
named places/businesses → find_place (returns lat/lon directly). \
...
glowing golden circle hugs the point. Then say what it is in one or two sentences. For a named \
business/venue/address you don't have exact coordinates for, call find_place FIRST, fly to the top \
match (state its name + address) — all five boroughs are fully built, so anywhere in NYC is fair game.
...
async function runTool(name, input) {
  try {
    ...
    if (name === 'find_place') return await findPlace(input || {});
